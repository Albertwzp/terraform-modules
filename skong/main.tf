...
resource "kong_plugin" "plugin" {
  ...
  route_id    = jsondecode(file("${each.key}")).route == null ? null : kong_route.route[replace(replace(replace("${each.key}", format("%s/", split("/", "${each.key}")[1]), ""), "plugins", "routes"), "-plugin", "-route")].id
